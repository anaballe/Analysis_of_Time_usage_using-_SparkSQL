...
object TimeUsage {
  ...
  /** @return The initial data frame columns partitioned in three groups: primary needs (sleeping, eating, etc.),
    ...
    */
  def classifyColumnRec(columnNames :List[String],primary :List[Column],working :List[Column],leisure :List[Column]):(List[Column],List[Column],List[Column]) = columnNames match {
    ...
    case x::xs =>
      if(x.startsWith("t01") || x.startsWith("t03") || x.startsWith("t11") || x.startsWith("t1801") || x.startsWith("t1803"))
        classifyColumnRec(xs,col(x)::primary,working,leisure)
      else if (x.startsWith("t05") || x.startsWith("t1805"))
        classifyColumnRec(xs, primary, col(x)::working, leisure)
        ...
        classifyColumnRec(xs, primary, working,  col(x)::leisure)
      else
        classifyColumnRec(xs, primary, working, leisure)
  }
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    classifyColumnRec(columnNames,primary,working,leisure)
  }
